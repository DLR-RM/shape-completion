...
def _read_array(store: Any, key: str) -> np.ndarray:
...
def _load_database_helpers() -> tuple[Any, Any]:
    database_module = importlib.import_module("database")
    return cast(Any, database_module).COLMAPDatabase, cast(Any, database_module).blob_to_array


def _load_dense_io_helpers() -> tuple[Any, Any]:
    dense_io_module = importlib.import_module("read_write_dense")
    return cast(Any, dense_io_module).read_array, cast(Any, dense_io_module).write_array
...
def get_camera(cam_id: int, camera_model: str, width: int, height: int, calibration: Any) -> list[Any]:
    K = _read_array(calibration, f"N{cam_id}_rgb_K")
    d = _read_array(calibration, f"N{cam_id}_rgb_d")
...
def get_image(cam_id: int, angle: int, image_counter: int, data_dir: str, calibration: Any) -> list[Any]:
    image_id = angle // 3 + 1
    image_name = "image" + str(image_id).zfill(3) + ".jpg"

    with h5py.File(os.path.join(data_dir, "poses", f"NP5_{angle}_pose.h5")) as pose_data:
        f_table_ref = _read_array(pose_data, "H_table_from_reference_camera")

    f_rgb_ref = _read_array(calibration, f"H_N{cam_id}_from_NP5")
...
def get_mask(data_dir: str, image_path: str, pad: list[int], image: np.ndarray, show_bbox: bool = False) -> np.ndarray:
    image_id = int(image_path.split("/")[-1].split("_")[-1].split(".")[0]) // 3 + 1
    init_mask_path = os.path.join(data_dir, "masks", image_path.split("/")[-1].replace(".jpg", "_mask.pbm"))
    ...
    if show_bbox and image_id == 1:
        import cv2

        cv2.imshow("Bounding Box", cv2.resize(rembg_input, (rembg_input.shape[1] // 2, rembg_input.shape[0] // 2)))
        cv2.waitKey(0)
        cv2.destroyAllWindows()

    rembg_output = np.asarray(
        remove(rembg_input, alpha_matting=True, alpha_matting_erode_size=15, only_mask=True),
        dtype=np.uint8,
    )
...
def main(args: Any):
    ...
    if args.data or (args.sparse and not os.path.isfile(database_path)):
        ...
        for cam_id in args.camera_ids:
            ...
            for angle in tqdm(np.arange(0, 360, 3), desc=f"Processing images [camera {cam_id}/{len(args.camera_ids)}]"):
                ...
                images.append(get_image(cam_id, int(angle), image_counter, data_dir, calibration))
                image_counter += 1
                ...
                if not os.path.isfile(refined_mask_path) or mask_depth or masked:
                    try:
                        ...
                        mask = get_mask(data_dir, image_path, pad, image, show_bbox=args.show_bbox)
                        Image.fromarray(mask).save(refined_mask_path)

                if mask_depth:
                    Image.fromarray(mask).save(depth_mask_path, quality=100, subsampling=0)

                if masked:
                    os.makedirs(masked_dir, exist_ok=True)
                    masked_image = Image.composite(
                        pil_image, Image.fromarray(np.zeros_like(image)), Image.fromarray(mask)
                    )
                    masked_image.save(masked_path, quality=100, subsampling=0, exif=pil_image.info["exif"])
    ...
    if args.sparse and not os.path.isfile(os.path.join(sparse_dir, "database.db")):
        ...
        COLMAPDatabase, blob_to_array = _load_database_helpers()
        ...
        for db_image, image in zip(db_images, new_images, strict=True):
        ...
        for db_cam, cam in zip(db_cams, cameras, strict=True):
            camera_id, model, width, height, params, _prior = db_cam
            if args.update_cameras:
    ...
    if args.show_sparse and (sparse_exists or sparse_will_exist):
        pipeline.append(
            f"python {args.colmap_github_dir}/scripts/python/visualize_model.py"
    ...
    for step in pipeline:
        if args.mask_depth and "stereo_fusion" in step:
            read_array, write_array = _load_dense_io_helpers()
            for cam_id in args.camera_ids:
                ...
                for depth_path in tqdm(
                    ...
                    if not os.path.isfile(depth_copy_path) or not os.path.isfile(normals_copy_path):
                        ...
                        if os.path.isfile(mask_path):
                            ...
                            Image.fromarray(mask).save(mask_path, quality=100, subsampling=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1600,
        help="Maximum estimated depth map size.Increase to obtain denser output for weakly textured surfaces.",
    )
    parser.add_argument(
        "--window_radius",
        type=int,
    ...
    parser.add_argument(
        "--mesh", action="store_true", help="Run Poisson surface reconstruction on the dense MVS output."
    )
    parser.add_argument(
        "--fast_depth", type=int, default=True, help="Perform fast but potentially less accurate depth estimation."
    )
    ...
    parser.add_argument(
        "--update_cameras", type=int, default=True, help="Updates camera parameters from calibration data file."
    )
    parser.add_argument(
        "--show_bbox",
        action="store_true",
    ...
    parser.add_argument(
        "--multiple_models", action="store_true", help="Allow reconstruction of multiple partial models."
    )
    parser.add_argument(
        "--camera_ids", nargs="*", type=int, default=[1, 2, 3], help="Cameras to include in reconstruction."
    )
