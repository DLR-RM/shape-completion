...
def normalize(in_path: Path, out_dir: Path, args: Any):
    ...
    logger.debug(f"Normalizing mesh took {time.perf_counter() - restart:.2f}s.")
    restart = time.perf_counter()
    save_mesh(out_path, mesh.vertices, mesh.faces)
    logger.debug(f"Saving mesh took {time.perf_counter() - restart:.2f}s.")
...
def sample(in_path: Path, out_dir: Path, args: Any):
    ...
    if (
        ...
        and uniform_points_path.exists()
        and surface_points_path.exists()
    ...
    if len(mesh.vertices) == 0 or len(mesh.faces) == 0:
        ...
        return
    logger.debug(f"Loaded mesh in {time.perf_counter() - restart:.4f}s.")
    ...
    if not mesh.is_watertight:
        if args.process:
            ...
            logger.debug(f"Processed mesh in {time.perf_counter() - restart:.4f}s.")
        if not mesh.is_watertight:
    ...
    if not pointcloud_path.exists() or args.overwrite:
        ...
        logger.debug(f"Sampled pointcloud in {time.perf_counter() - restart:.4f}s.")
    ...
    if args.show and not any([pcd is None, normals is None, points is None, occupancy is None]):
        vis = Visualize(show_inputs=False, show_mesh=True, show_pointcloud=True)
        vis(
            {
                "obj_name": in_path.stem,
...
def main():
    ...
    parser.add_argument("--out_format", type=str, choices=[".off", ".obj", ".ply"], help="Output file format.")
    ...
    parser.add_argument("--normalize", action="store_true", help="Normalize mesh(es) to unit cube.")
    ...
    parser.add_argument("--process", action="store_true", help="Process meshes before sampling.")
