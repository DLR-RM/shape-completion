...
from ..src.utils import part_sphere
...
def render(in_path: Path, out_dir: Path, args: Any):
    ...
    intrinsic = np.array([[args.fx, 0, args.cx], [0, args.fy, args.cy], [0, 0, 1]])
    ...
    if len(vertices) == 0 or len(faces) == 0:
        ...
        return
    logger.debug(f"Loaded mesh ({len(vertices)} vertices, {len(faces)} faces) in {time.perf_counter() - restart:.4f}s.")
    ...
    logger.debug(f"Normalized mesh in {time.perf_counter() - restart:.4f}s.")
    ...
    logger.debug(f"Prepared scene in {time.perf_counter() - restart:.4f}s.")

    restart = time.perf_counter()
    renderer = pyrender.OffscreenRenderer(args.width, args.height)
    shader_dir = Path(__file__).parent.parent / "utils" / "assets" / "shaders"
    pyrender_internal = cast(Any, renderer._renderer)
    pyrender_internal._program_cache = ShaderProgramCache(shader_dir=shader_dir)
    logger.debug(f"Prepared renderer in {time.perf_counter() - restart:.4f}s.")

    restart = time.perf_counter()
    kinect_sim = KinectSim()
    logger.debug(f"Prepared Kinect simulator in {time.perf_counter() - restart:.4f}s.")
    ...
    for index in range(args.n_views):
        ...
        inplane_rot = (
            R.from_euler("z", args.inplane_rotation, degrees=True).as_matrix() if args.inplane_rotation else None
        )
        ...
        for _ in range(100):
            ...
            eye = part_sphere(center=np.zeros(3), radius=radius, mode="SURFACE", part_sphere_dir_vector=[0, 1, 0])
            ...
            render_out = renderer.render(scene, flags=pyrender.RenderFlags.OFFSCREEN)
            if render_out is None:
                raise RuntimeError("Pyrender returned no render output.")
            if isinstance(render_out, tuple):
                normal_image = np.asarray(render_out[0], dtype=np.uint8)
                depth_map = np.asarray(render_out[1], dtype=np.float32)
            else:
                depth_map = np.asarray(render_out, dtype=np.float32)
                normal_image = np.zeros((args.height, args.width, 3), dtype=np.uint8)
            logger.debug(f"Rendered depth and normal map in {time.perf_counter() - restart:.4f}s.")

            restart = time.perf_counter()
            extrinsic = np.asarray(inv_trafo(inv_extrinsic)).copy()
            extrinsic[1, :] *= -1
            extrinsic[2, :] *= -1
            tmp_mesh.apply_transform(extrinsic)
            vertices, faces = tmp_mesh.vertices, tmp_mesh.faces
            kinect_depth_map = kinect_sim.simulate(
                vertices, faces, args.width, args.height, args.fx, args.fy, args.cx, args.cy, args.verbose
            )
            tmp_mesh.apply_transform(inv_trafo(extrinsic))
            logger.debug(f"Simulated Kinect depth in {time.perf_counter() - restart:.4f}s.")
        ...
        logger.debug(f"Processed depth and normal map in {time.perf_counter() - restart:.4f}s.")
        ...
        logger.debug(f"Processed Kinect depth map in {time.perf_counter() - restart:.4f}s.")
        ...
        logger.debug(f"Saved depth and normal map in {time.perf_counter() - restart:.4f}s.")
        ...
        logger.debug(f"Saved Kinect depth map in {time.perf_counter() - restart:.4f}s.")
        ...
        if args.show:
            ...
            pcd = convert_rgbd_image_to_point_cloud(
                [str(normal_path), str(depth_path)],
                intrinsic,
                cast(np.ndarray, extrinsic),
                depth_scale=ds,
                depth_trunc=args.zfar,
                convert_rgb_to_intensity=False,
            )
            pcd.normals = o3d.utility.Vector3dVector(np.asarray(pcd.colors) * 2 - 1)
            logger.debug(f"Converted depth and normal map to point cloud in {time.perf_counter() - restart:.4f}s.")

            restart = time.perf_counter()
            kinect_ds = 1 / kinect_depth_max if args.depth_precision == 8 else kinect_depth_scale
            kinect_pcd = convert_depth_image_to_point_cloud(
                str(kinect_depth_path),
                intrinsic,
                cast(np.ndarray, extrinsic),
            ...
            logger.debug(f"Converted Kinect depth map to point cloud in {time.perf_counter() - restart:.4f}s.")

            if args.verbose:
                o3d_visualization = cast(Any, o3d).visualization
                o3d_visualization.draw_geometries([copy.deepcopy(kinect_pcd).paint_uniform_color([1, 0, 0]), pcd])
            o3d_visualization = cast(Any, o3d).visualization
            o3d_visualization.draw_geometries(
                [
                    ...
                    o3d.geometry.TriangleMesh(
                        ...
                        triangles=o3d.utility.Vector3iVector(tmp_mesh.faces),
                    ...
                    #     inv_extrinsic),
                    o3d.geometry.TriangleMesh().create_coordinate_frame(size=0.1).transform(inv_trafo(extrinsic)),
                ]
            )
    ...
    np.savez_compressed(str(params_path), **cast(dict[str, Any], data))
