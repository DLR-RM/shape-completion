...
def normalize_mesh(input_path: str | Path, output_path: str, args: Any) -> tuple[float, float]:
    path = str(input_path)
    """
    ...
    if "ShapeNet.v1" in path and "03797390" in path:
        ids = [
...
def mesh_from_sdf(sdf_path: str, args: Any = None, **kwargs: Any) -> bool:
    output_path_arg = kwargs.get("o")
    if output_path_arg is None:
        raise ValueError(f"Missing output path for mesh extraction: {sdf_path}")
    ...
    if kwargs.get("method") == "vega":
        ...
        vertices, faces, normals, _ = marching_cubes(
            volume=volume, level=level, spacing=voxel_size, step_size=1, allow_degenerate=False, method="lewiner"
        )  # lorensen or lewiner
...
def uniform_grid_sampling(
    grid: np.ndarray,
    bounds: np.ndarray | list[Any] | tuple[Any, ...],
    num_points: int,
    mask: np.ndarray | list[Any] | tuple[Any, ...] | None = None,
) -> np.ndarray:
...
def sample(sdf_path: str, mesh_path: str, args: Any) -> None:
    ...
    if not isinstance(loaded_mesh, trimesh.Trimesh):
        raise TypeError(f"Expected Trimesh at {mesh_path}, got {type(loaded_mesh).__name__}")
    mesh = loaded_mesh
    if args.visualize:
        mesh.show()
    ...
    if args.voxel:
        ...
        if args.overwrite or not os.path.isfile(voxel_path):
            ...
            voxel = trimesh_binvox.voxelize_mesh(
                mesh, dimension=32, remove_internal=False, center=True, binvox_path=binvox_path
            )

            binvox = trimesh_binvox.export_binvox(voxel)  # Writes in 'xzy' format by default
    ...
    if name in args.samples and (args.overwrite or not os.path.isfile(sample_path.replace(replace, name))):
        ...
        deepsdf_samples = np.concatenate(
            [
                deepsdf_samples,
                uniform_random_sampling(sdf_values, sdf_bounds, num_points=args.num_points - len(deepsdf_samples)),
            ]
        )
        sample_names.append(name)
    ...
    if name in args.samples:
        for radius in args.uniform_sphere_radii:
            ...
            if args.overwrite or not os.path.isfile(sample_path.replace(replace, name)):
                ...
                uniform_sphere_samples = uniform_random_sampling(
                    sdf_values, sdf_bounds, points=sphere_points, ignore_bounds=True
                )
                sample_names.append(name)
    ...
    for samples, name in zip(all_samples, sample_names, strict=True):
...
def load_sdf(sdf_path: str, resolution: int = 256) -> SdfData:
    intsize = 4
    floatsize = 8
    with open(sdf_path, "rb") as f:
        data = f.read()
    ress = np.frombuffer(data[: intsize * 3], dtype=np.int32)
    ...
    positions = np.frombuffer(data[intsize * 3 : intsize * 3 + floatsize * 6], dtype=np.float64)
    bounds = np.asarray(positions[:6], dtype=np.float32)
    values = np.frombuffer(data[intsize * 3 + floatsize * 6 :], dtype=np.float32)
    values = np.reshape(values, (resolution + 1, resolution + 1, resolution + 1))
    return {"bounds": bounds, "values": values}


def get_mesh_path(mesh_path: str | Path, args: Any) -> str:
...
def get_sample_path(sdf_path: str, args: Any) -> str:
    if not args.shapenet:
...
def run(mesh: str | Path, args: Any) -> None:
    mesh = str(mesh)
    ...
    expected_num_samples = (
        ...
        + len(list(set(args.samples) & {"uniform_sphere", "uniform_random"}))
        + 2
    )
    ...
    if sdf_exists and mesh_exists and samples_exist and not args.overwrite:
        _debug_level_1(f"Everything done. Skipping mesh {mesh}")
        return
    ...
    if not sdf_exists or args.overwrite:
        try:
            _debug_level_1(f"Saving SDF to: {sdf_path}")
            kwargs = {
                "n": args.n,
            ...
            _debug_level_1(f"SDF time: {time.perf_counter() - start}")
        except Exception as e:
            logger.exception(e)
            return
    else:
        _debug_level_1(f"SDF computation for {mesh} done. Skipping.")

    mesh_path = mesh_path.replace(".obj", ".ply")
    if not mesh_exists or args.overwrite:
        try:
            _debug_level_1(f"Saving mesh from marching cubes to: {mesh_path}")
            kwargs = {
                "o": mesh_path,
                "i": 0.0,
                "n": args.n,
            ...
            _debug_level_1(f"Marching cubes time: {time.perf_counter() - start}")
        except Exception as e:
            logger.exception(e)
            return
    else:
        _debug_level_1(f"Marching cubes for {mesh} done. Skipping.")

    if not samples_exist or args.overwrite:
        try:
            start = time.perf_counter()
            sample(sdf_path, mesh_path, args)
            _debug_level_1(f"Sample time: {time.perf_counter() - start}")
        except Exception as e:
            logger.exception(e)
            return
    else:
        _debug_level_1(f"Sampling for mesh {mesh} done. Skipping.")
    ...
    for item in test:
        if item.split(".")[-1] not in keep:
            path = os.path.join(dir_name, item)
            _debug_level_1(f"Removing {path}")
            os.remove(path)

    _debug_level_1(f"Runtime: {time.perf_counter() - start_run}")
...
def main():
    ...
    parser.add_argument(
        "--num_points", type=int, default=int(1e5), help="Number of points to sample from the SDF grid."
    )
    parser.add_argument("--num_uniform_random", type=int, default=1, help="Number of uniform random samples to take.")
    parser.add_argument(
        "--uniform_sphere_radii",
        nargs="*",
        type=float,
        default=[1, 2, 5, 10],
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        default=["uniform_random.npy", "uniform_sphere.npy"],
    ...
    parser.add_argument(
        "--method", type=str, default="skimage", help="Marching cubes method (one of 'skimage' or 'vega')."
    )
    ...
    save_command_and_args_to_file(args.o if args.o else Path.cwd() / "command.txt", args)

    meshes = args.meshes
    if len(meshes) == 1:
        if "*" in meshes[0]:
            meshes = glob(meshes[0])
        else:
            meshes = [resolve_path(meshes[0])]
    else:
    ...
    print(
        ...
        len(meshes),
        "mesh(es) using",
        cpu_count() if args.parallel == -1 else min(args.parallel, cpu_count()),
        "processes.",
    )
