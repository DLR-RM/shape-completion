...
def build_vhacd() -> Path:
    ...
    if not binary_path.exists():
        ...
        os.system(f"cd {vhacd_path!s}/v-hacd/app; cmake -S . -B build -DCMAKE_BUILD_TYPE=Release; cmake --build build")
...
def duplicate(r1: np.ndarray, r2: np.ndarray, up_axis: int = 2, tolerance: float = 1.0) -> bool:
...
def simplify_types(value: str) -> bool | int | float:
    if value == "True":
        return True
    elif value == "False":
        return False
    try:
        ...
        try:
            ...
            raise ArgumentTypeError(
                "Invalid value provided for --simplify. Must be one of [True, False, int, float]"
            ) from err


def show_types(value: str) -> bool | str:
    if value == "True":
        return True
    elif value == "False":
        return False
...
def simulate(file: Path, args: Namespace, p: bullet_client.BulletClient | None = None):
    ...
    if args.simplify:
        ...
        if isinstance(args.simplify, float):
            target_num_faces = int(args.simplify * num_faces)
        elif isinstance(args.simplify, int):
        ...
        logger.debug(f"Simplifying mesh took {time.perf_counter() - restart:.2f}s.")
    ...
    if args.solidify:
        ...
        bpy_data = cast(Any, bpy.data)
        bpy_context = cast(Any, bpy.context)
        bpy_objects = cast(Any, bpy_data.objects)

        obj_path = file.parent / "solidified.obj"
        if not obj_path.exists():
            logger.debug("Solidifying mesh.")
            bpy.ops.object.select_all(action="SELECT")
            bpy.ops.object.delete(use_global=False)

            bpy_mesh = cast(Any, bpy_data.meshes.new("Mesh"))
            bpy_mesh.from_pydata(obj_mesh.vertices.tolist(), [], obj_mesh.triangles.tolist())
            bpy_mesh.update()
            bpy_mesh.validate()

            bpy_obj = bpy_objects.new("Object", bpy_mesh)
            collection = getattr(bpy_context, "collection", None)
            if collection is None:
                raise RuntimeError("No active Blender collection; cannot solidify mesh.")
            collection.objects.link(bpy_obj)

            bpy_obj_ref = bpy_objects["Object"]
            bpy_obj_ref.select_set(True)
            view_layer = getattr(bpy_context, "view_layer", None)
            if view_layer is None:
                raise RuntimeError("No active Blender view layer; cannot solidify mesh.")
            view_layer.objects.active = bpy_obj_ref

            solidify = cast(Any, bpy_obj_ref.modifiers.new(name="Solidify", type="SOLIDIFY"))
            solidify.thickness = 0.001
            bpy.ops.object.modifier_apply(modifier=solidify.name)

            triangulate = cast(Any, bpy_obj_ref.modifiers.new(name="Triangulate", type="TRIANGULATE"))
            bpy.ops.object.modifier_apply(modifier=triangulate.name)

            bpy_mesh = cast(Any, bpy_obj_ref.data)
            vertices = np.array([v.co for v in bpy_mesh.vertices])
            faces = np.array([poly.vertices[:] for poly in bpy_mesh.polygons])
            ...
            obj_mesh = Trimesh(*load_mesh(obj_path))
        logger.debug(f"Solidifying mesh took {time.perf_counter() - restart:.2f}s.")
    ...
    if args.center:
        ...
        save_mesh(obj_path, obj_mesh.vertices, obj_mesh.faces)
        logger.debug(f"Centering mesh took {time.perf_counter() - restart:.2f}s.")
        ...
        if not vhacd_path.exists():
            ...
            try:
                ...
                with stdout_redirected(enabled=not args.verbose):
                    p.vhacd(str(obj_path), str(vhacd_path), str(log_path), resolution=int(1e6))
            log_path.unlink(missing_ok=True)
        else:
            logger.debug(f"Found V-HACD mesh at {vhacd_path}.")
        logger.debug(f"V-HACD took {time.perf_counter() - restart:.2f}s.")

    restart = time.perf_counter()
    p.resetSimulation()
    p.setAdditionalSearchPath(pybullet_data.getDataPath())
    p.loadURDF("plane100.urdf", useMaximalCoordinates=True, useFixedBase=True)
    ...
    def get_pose(rot: R) -> np.ndarray:
        ...
        mesh = mesh.apply_transformations(trafo)
        ...
        _, quat = p.getBasePositionAndOrientation(bodyUniqueId=object_id)
        trafo = np.eye(4)
    ...
    logger.debug(f"PyBullet simulation took {time.perf_counter() - restart:.2f}s.")
    logger.debug(f"Total time: {time.perf_counter() - start:.2f}s.")

    if args.show == "poses":
        o3d_visualization = cast(Any, o3d).visualization
        obj_mesh = o3d.geometry.TriangleMesh(
            o3d.utility.Vector3dVector(obj_mesh.vertices), o3d.utility.Vector3iVector(obj_mesh.faces)
        )
        obj_mesh.compute_vertex_normals()
        rot_x_90 = R.from_euler("x", -np.pi / 2).as_matrix()
        for pose in poses:
            pose = rot_x_90 @ pose[:3, :3]
            o3d_visualization.draw_geometries(
                [
                    copy.deepcopy(obj_mesh).rotate(pose, center=[0, 0, 0]),
                    o3d.geometry.TriangleMesh().create_coordinate_frame(size=0.1),
                ]
            )


def run(file: Path, args: Namespace, p: bullet_client.BulletClient | None = None):
    start = time.perf_counter()
    try:
...
def main():
    ...
    with tqdm_joblib(tqdm(desc="Simulating", total=len(files), disable=args.verbose)):
        Parallel(n_jobs=1 if args.show or args.verbose else min(args.n_jobs, len(files)), verbose=args.verbose)(
            delayed(run)(file, args, p) for file in files
        )
