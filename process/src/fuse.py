...
def render(
    ...
    for rotation in rotations:
        ...
        render_out = renderer.render(scene, flags=pyrender.RenderFlags.DEPTH_ONLY)
        if isinstance(render_out, tuple):
            depth = np.asarray(render_out[1], dtype=np.float32)
        elif render_out is None:
            raise RuntimeError("Pyrender returned no depth output.")
        else:
            depth = np.asarray(render_out, dtype=np.float32)
        depth = depth.copy()
...
def fuse(
    ...
    Ks = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]]).reshape((1, 3, 3))
    ...
    depthmaps_arr = np.array(depthmaps).astype(np.float32)
    voxel_size = 1 / resolution

    views = PyViews(depthmaps_arr, Ks, Rs, Ts)
    tsdf = tsdf_fusion(
        views,
        depth=resolution,
        height=resolution,
...
def pyfusion_pipeline(mesh: Trimesh | dict[str, np.ndarray], args: Any) -> Trimesh | dict[str, np.ndarray]:
    start = time.perf_counter()
    rotations = get_views(get_points(args.n_views))
    logger.debug(f"Generated rotations in {time.perf_counter() - start:.3f}s.")
    ...
    logger.debug(f"Rendered depth maps in {time.perf_counter() - restart:.3f}s.")

    restart = time.perf_counter()
    tsdf = fuse(depthmaps, rotations, args.resolution, args.fx, args.fy, args.cx, args.cy)
    logger.debug(f"Fused depth maps in {time.perf_counter() - restart:.3f}s.")

    restart = time.perf_counter()
    mesh = extract(
        grid=-tsdf, level=0, resolution=args.resolution, return_type="trimesh" if args.use_trimesh else "dict"
    )
    logger.debug(f"Extracted mesh in {time.perf_counter() - restart:.3f}s.")
    return mesh
