...
def preprocess(model: Any, return_center_scale: bool = False) -> Any | tuple[Any, np.ndarray, float]:
...
def voxel_carving(
    ...
    # rescale geometry
    camera_sphere = cast(Any, preprocess(camera_sphere))
    mesh = cast(Any, preprocess(mesh))

    # setup visualizer to render depthmaps
    o3d_visualization = cast(Any, o3d).visualization
    vis = o3d_visualization.Visualizer()
    ...
    for cid, xyz in enumerate(camera_sphere.vertices):
        ...
        if cid == 50:
            o3d_visualization = cast(Any, o3d).visualization
            o3d_visualization.draw_geometries([depth])

        pcd_agg += o3d.geometry.PointCloud().create_from_depth_image(
            o3d.geometry.Image(depth), param.intrinsic, param.extrinsic, depth_scale=1
        )
        ...
        print(f"Carve view {cid + 1:03d}/{len(camera_sphere.vertices):03d}")
    vis.destroy_window()

    # add voxel grid surface
    print(f"Surface voxel grid from {surface_method}")
...
def voxel_carving_pipeline(
    mesh: Trimesh | dict[str, np.ndarray], cubic_size: float = 2.0, voxel_resolution: float = 64.0
) -> dict[str, np.ndarray]:
    ...
    mesh_o3d, _center, _scale = cast(tuple[Any, np.ndarray, float], preprocess(mesh_o3d, return_center_scale=True))

    vertices = np.asarray(cast(Any, mesh_o3d).vertices)
    ...
    _voxel_grid, carved_voxels, _voxel_surface = voxel_carving(
        mesh_o3d, output_filename, camera_path, cubic_size, voxel_resolution
    )
    print("Carved voxels ...")
    print(carved_voxels)
    ...
    o3d_visualization = cast(Any, o3d).visualization
    o3d_visualization.draw_geometries([carved_mesh])
