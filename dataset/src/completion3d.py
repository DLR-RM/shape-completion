...
from .fields import PointCloudField, PointsField
from .transforms import Transform

logger = setup_logger(__name__)

try:
    transforms3d = import_module("transforms3d")
except ImportError as e:
    transforms3d = None
    logger.debug(f"Transforms3D import failed. Original Completion3D transforms won't be available: {e}")
...
def augment_cloud(Ps, pc_augm_scale, pc_augm_rot, pc_augm_mirror_prob, pc_augm_jitter):
    if transforms3d is None:
        raise RuntimeError("transforms3d is required for augment_cloud but is not installed.")
...
class Completion3D(Dataset):
    def __init__(
        ...
        classmap = pd.read_csv(
            os.path.join(data_dir, "synsetoffset2category.txt"),
            dtype=str,
            sep=r"\s+",
            header=None,
        )
        self.classmap = dict(zip(classmap.values[:, 1], classmap.values[:, 0], strict=False))
        ...
        self.data_paths = sorted(
            [
                os.path.join(data_dir, self.split, "partial", k.rstrip() + ".h5")
                for k in open(os.path.join(data_dir, f"{self.split}.list")).readlines()
                if k.split("/")[-2] in categories
            ]
        )
        self.objects = list()
        ...
        self.points_trafos = Compose(transforms["points"]) if transforms and transforms.get("points") else None
    ...
    def load_data(self, filename: str):
        ...
        if self.load_pointcloud:
            pointcloud = self.pointcloud.load(
                obj_dir=os.path.join(self.shapenet_dir, category, obj_name), index=0, category=category
            )
        else:
            pointcloud = None
        # if self.split == "train":
        ...
        if self.load_points:
            points = self.points.load(
                obj_dir=os.path.join(self.shapenet_dir, category, obj_name), index=0, category=category
            )
            if self.points_trafos is not None:
                points = self.points_trafos(points)
        else:
            points = None
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        inputs, points, pointcloud, pcd2048, _category, obj_name = self.load_data(self.data_paths[index])
        data = {
            "index": index,
            "obj_name": obj_name,
