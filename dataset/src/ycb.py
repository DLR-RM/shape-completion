...
from .fields import BlenderProcRGBDField, MeshField, PointCloudField, PointsField, RGBDField, VoxelsField
from .transforms import RotatePointcloud, Transform

# Field constructors accept additional kwargs via shared base classes;
# cast to Any here to keep pyright from over-restrictive call signatures.
RGBDFieldAny = cast(Any, RGBDField)
PointCloudFieldAny = cast(Any, PointCloudField)
BlenderProcRGBDFieldAny = cast(Any, BlenderProcRGBDField)
PointsFieldAny = cast(Any, PointsField)
VoxelsFieldAny = cast(Any, VoxelsField)
MeshFieldAny = cast(Any, MeshField)
...
class YCB(Dataset):
    def __init__(
        ...
        objects: Iterable[str] | Iterable[int] = (6, 19, 21),
        split: str = "train",
        rgb: bool = False,
        ...
        self.metadata = {
            obj["category"]: {"index": index, "name": obj["name"][4:]} for index, obj in enumerate(self.objects)
        }
        ...
        self.fields = dict()
        if load_real_data:
            self.fields["inputs"] = RGBDFieldAny(
                rgb=rgb or input_type in ["image", "rgbd"],
            ...
            if input_type != "image":
                ...
                self.fields["inputs"] = PointCloudFieldAny(
                    file=pointcloud_file if pointcloud_file else mesh_file,
                    data_dir=mesh_dirname,
                    cache=True,
                    transform=Compose(inputs_trafos) if inputs_trafos else None,
                )
            if input_type in ["image", "rgbd"]:
                path_prefix = ""  # Set to local BlenderProc RGBD output path
                self.fields["inputs"] = BlenderProcRGBDFieldAny(
        ...
        if load_points:
            ...
            self.fields["points"] = PointsFieldAny(
                file=points_file if points_file else mesh_file,
                data_dir=mesh_dirname,
                padding=padding,
        ...
        if load_pointcloud:
            ...
            self.fields["pointcloud"] = PointCloudFieldAny(
                file=pointcloud_file if pointcloud_file else mesh_file,
                data_dir=mesh_dirname,
        ...
        if voxels_file is not None:
            self.fields["voxels"] = VoxelsFieldAny(file=voxels_file)

        if load_meshes:
            self.fields["mesh"] = MeshFieldAny(file=mesh_file, data_dir=mesh_dirname, pose_file=pose_file, cache=True)
