# Copied from https://github.com/NVIDIA/NeMo/blob/main/nemo/collections/common/callbacks/ema.py
...
class EMACallback(pl.Callback):
    ...
    def save_ema_model(self, trainer: "pl.Trainer") -> Iterator[None]:
        """
    ...
    @contextlib.contextmanager
    def save_original_optimizer_state(self, trainer: "pl.Trainer") -> Iterator[None]:
        for optimizer in trainer.optimizers:
            assert isinstance(optimizer, EMAOptimizer)
            ema_optimizer = cast(EMAOptimizer, optimizer)
            ema_optimizer.save_original_optimizer_state = True
        try:
            yield
        finally:
            for optimizer in trainer.optimizers:
                ema_optimizer = cast(EMAOptimizer, optimizer)
                ema_optimizer.save_original_optimizer_state = False
    ...
    def on_load_checkpoint(
        ...
        # Avoid direct access to checkpoint connector internals.
        ckpt_path_obj = trainer.ckpt_path
        ckpt_path = str(ckpt_path_obj) if ckpt_path_obj is not None else None

        if ckpt_path and checkpoint_callback is not None and "NeMo" in type(checkpoint_callback).__name__:
            ext = getattr(checkpoint_callback, "FILE_EXTENSION", ".ckpt")
...
class EMAOptimizer(torch.optim.Optimizer):
    ...
    def update(self) -> None:
        ...
        stream = cast(torch.cuda.Stream, self.stream)
    ...
    def switch_main_parameter_weights(self, saving_ema_model: bool = False) -> None:
        ...
        for param, ema_param in zip(self.all_parameters(), self.ema_params, strict=True):
