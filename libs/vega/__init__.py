...
if not os.environ.get("LD_LIBRARY_PATH", False) or str(vega_lib_dir) not in os.environ["LD_LIBRARY_PATH"]:
    os.environ["LD_LIBRARY_PATH"] = f"$LD_LIBRARY_PATH:{vega_lib_dir!s}"
...
def compute_marching_cubes(sdf_path: str, output_path: str, **kwargs: Any):
    ...
    mesh = cast(Any, trimesh.load(vega_output_path, force="mesh", process=False, validate=False))
    mesh.export(
        output_path,
