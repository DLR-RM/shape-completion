...
class PVConv(nn.Module):
    ...
    def forward(self, features: Tensor | tuple[Tensor, Tensor], inputs: Tensor) -> tuple[Tensor, Tensor]:
        if not isinstance(features, Tensor):
            features, _ = features
        voxel_features, voxel_coords = self.voxelization(features, inputs)
        voxel_features = self.voxel_layers(voxel_features)
        voxel_features_samples = trilinear_devoxelize(voxel_features, voxel_coords, self.resolution, self.training)
