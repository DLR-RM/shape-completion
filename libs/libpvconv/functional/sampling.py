...
class Gather(Function):
    ...
    def forward(ctx, features, indices):
        ...
        return _ext_backend.gather_features_forward(features, indices)

    @staticmethod
    def backward(ctx, grad_output):
        (indices,) = ctx.saved_tensors
        grad_features = _ext_backend.gather_features_backward(grad_output.contiguous(), indices, ctx.num_points)
...
def furthest_point_sample(
    ...
    torch_gather: bool = False,
) -> torch.Tensor:
    if not channels_first:
        points = points.transpose(1, 2)
    indices = _ext_backend.furthest_point_sampling(points.contiguous().float(), num_samples)
    if return_indices:
        return indices.long()
    if torch_gather:
        expanded_indices = indices.unsqueeze(1).expand(-1, points.size(1), -1)
        fps = torch.gather(points.float(), 2, expanded_indices.long())
    else:
        fps = cast(torch.Tensor, gather(points.float(), indices))
    if not channels_first:
        return fps.transpose(1, 2)
    return fps.type(points.dtype)
