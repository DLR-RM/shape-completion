...
def get_voxel_coords(
    ...
    if normalize:
        norm_coords = (
            norm_coords / (norm_coords.norm(dim=1, keepdim=True).max(dim=2, keepdim=True).values * 2.0 + eps) + 0.5
        )
    else:
...
class Voxelization(nn.Module):
    ...
    def forward(self, features: Tensor, points: Tensor) -> tuple[Tensor, Tensor]:
        ...
        voxels = cast(Tensor, avg_voxelize(features, torch.round(voxel_coords).to(torch.int32), self.r))
