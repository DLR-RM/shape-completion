...
try:
    ...
    warnings.warn('The `intersect` library is not installed.'
                  'Please install it using `python libs/libmanager.py install intersect`')
    raise
...
class MeshIntersector:
    def __init__(self, mesh, resolution=512, verbose=False):
