...
def main():
    ...
    for file in os.listdir(args.data_dir):
        if file.endswith(".npz"):
                ...
                if "ShapeNetCore" in args.mesh_dir or "bop" in args.mesh_dir:
                    ...
                    if "ShapeNetCore.v1" in args.mesh_dir:
                        ...
                        gt_mesh_path = os.path.join(
                            args.mesh_dir, args.data_dir.split("/")[-3].split("_")[0], f"obj_{mesh_id}.ply"
                        )
                    else:
                    ...
                    gt_mesh = trimesh.Trimesh(
                        vertices=np.asarray(gt_mesh.vertices), faces=np.asarray(gt_mesh.triangles), process=False
                    )
                    if "ShapeNetCore" in args.mesh_dir:
            ...
            for color, mesh in zip(color_list, mesh_list, strict=False):
