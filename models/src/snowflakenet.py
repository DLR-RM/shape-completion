...
from libs import ChamferDistanceL2, EarthMoversDistance, furthest_point_sample, gather, group

from .model import Model


def sample_and_group_all(
    xyz: Tensor, points: Tensor | None, use_xyz: bool = True
...
def query_knn(nsample: int, xyz: Tensor, new_xyz: Tensor, include_self: bool = True) -> Tensor:
...
def sample_and_group_knn(
    xyz: Tensor, points: Tensor | None, npoint: int, k: int, use_xyz: bool = True, idx: Tensor | None = None
) -> tuple[Tensor, Tensor, Tensor, Tensor]:
    """
    Args:
        xyz: Tensor, (B, 3, N)
    ...
    new_xyz = cast(Tensor, gather(xyz, furthest_point_sample(xyz_flipped, npoint)))  # (B, 3, npoint)
    if idx is None:
        idx = query_knn(k, xyz_flipped, new_xyz.permute(0, 2, 1).contiguous())
    assert idx is not None
    grouped_xyz = cast(Tensor, group(xyz, idx))  # (B, 3, npoint, nsample)
    grouped_xyz -= new_xyz.unsqueeze(3).repeat(1, 1, 1, k)

    if points is not None:
        grouped_points = cast(Tensor, group(points, idx))  # (B, f, npoint, nsample)
        if use_xyz:
...
class Conv2d(nn.Module):
    def __init__(
        self, in_channel, out_channel, kernel_size=(1, 1), stride=(1, 1), if_bn=True, activation_fn=torch.relu
    ):
        super().__init__()
...
class PointNet_SA_Module_KNN(nn.Module):
    def __init__(self, npoint, nsample, in_channel, mlp, if_bn=True, group_all=False, use_xyz=True, if_idx=False):
        ...
        last_channel = in_channel
        mlp_conv: list[nn.Module] = []
        for out_channel in mlp[:-1]:
            mlp_conv.append(Conv2d(last_channel, out_channel, if_bn=if_bn))
            last_channel = out_channel
        mlp_conv.append(Conv2d(last_channel, mlp[-1], if_bn=False, activation_fn=None))
    ...
    def forward(self, xyz, points, idx=None):
        ...
        """
        if self.group_all:
            new_xyz, new_points, idx, _grouped_xyz = sample_and_group_all(xyz, points, self.use_xyz)
        else:
            new_xyz, new_points, idx, _grouped_xyz = sample_and_group_knn(
...
class SkipTransformer(nn.Module):
    ...
    def forward(self, pos, key, query, include_self=True):
        ...
        b, _dim, n = value.shape

        pos_flipped = pos.permute(0, 2, 1).contiguous()
        idx_knn = query_knn(self.n_knn, pos_flipped, pos_flipped, include_self=include_self)

        key = group(key, idx_knn)  # b, dim, n, n_knn
        qk_rel = query.reshape((b, -1, n, 1)) - key

        pos_rel = pos.reshape((b, -1, n, 1)) - group(pos, idx_knn)  # b, 3, n, n_knn
...
class SPD(nn.Module):
    ...
    def forward(self, pcd_prev, feat_global=None, K_prev=None):
        ...
        """
        if self.global_feat and feat_global is None:
            raise ValueError("feat_global is required when global_feat=True")
        feat_global_tensor = cast(Tensor, feat_global)

        _b, _, _n_prev = pcd_prev.shape
        ...
        feat_1 = (
            torch.cat(
                [
                    ...
                    feat_global_tensor.repeat(1, 1, feat_1.size(2)),
                ],
                1,
            )
            if self.global_feat
...
class Transformer(nn.Module):
    def __init__(self, in_channel, dim=256, n_knn=16, pos_hidden_dim=64, attn_hidden_multiplier=4):
        """Transformer for point cloud feature learning"""
        super().__init__()
        self.n_knn = n_knn
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, point_cloud):
        ...
        l1_xyz, l1_points, _idx1 = self.sa_module_1(l0_xyz, l0_points)  # (B, 3, 512), (B, 128, 512)
        l1_points = self.transformer_1(l1_points, l1_xyz)
        l2_xyz, l2_points, _idx2 = self.sa_module_2(l1_xyz, l1_points)  # (B, 3, 128), (B, 256, 512)
        l2_points = self.transformer_2(l2_points, l2_xyz)
        _l3_xyz, l3_points = self.sa_module_3(l2_xyz, l2_points)  # (B, 3, 1), (B, out_dim, 1)

        return l3_points


class SeedGenerator(nn.Module):
    """Seed generator for point cloud completion"""
...
class Decoder(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512, radius=1, bounding=True, up_factors=None):
        super().__init__()
        self.num_p0 = num_p0
    ...
    def forward(self, feat, partial, return_P0=False) -> list[Tensor]:
        """
        Args:
        ...
        arr_pcd.append(pcd)
        pcd = cast(Tensor, furthest_point_sample(torch.cat([pcd, partial], 1), self.num_p0))
        if return_P0:
            arr_pcd.append(pcd)
        K_prev = None
...
class SnowflakeNet(Model):
    def __init__(self, **kwargs):
        ...
        self.decoder = Decoder(
            dim_feat=dim_feat, num_pc=num_pc, num_p0=num_p0, radius=radius, bounding=bounding, up_factors=up_factors
        )

        self.chamfer = ChamferDistanceL2()
        self.emd = EarthMoversDistance()

    def forward(self, inputs: Tensor, return_P0: bool = False, **kwargs) -> list[Tensor]:
        feature = self.encode(inputs)
        return self.decode(inputs, feature, return_P0=return_P0)

    def encode(self, inputs: Tensor, **kwargs) -> Tensor:
        pcd = inputs.permute(0, 2, 1).contiguous()
        return self.feat_extractor(pcd)

    def decode(self, inputs: Tensor, feature: Tensor, return_P0: bool = False, **kwargs) -> list[Tensor]:
        return self.decoder(feature, inputs, return_P0=return_P0)
    ...
    def loss(
        self,
        inputs: Tensor,
        pred_pcds: tuple[Tensor, Tensor, Tensor, Tensor],
        gt_pcd: Tensor,
        emd: bool = False,
        **kwargs,
    ) -> Tensor:
        pc, p1, p2, p3 = pred_pcds
        gt_2 = cast(Tensor, furthest_point_sample(gt_pcd, p2.shape[1]))
        gt_1 = cast(Tensor, furthest_point_sample(gt_2, p1.shape[1]))
        gt_c = cast(Tensor, furthest_point_sample(gt_1, pc.shape[1]))

        if emd:
            loss_c = self.emd(pc.repeat(1, 4, 1), gt_c.repeat(1, 4, 1))[0].sqrt().mean()
            loss_1 = self.emd(p1.repeat(1, 2, 1), gt_1.repeat(1, 2, 1))[0].sqrt().mean()
            loss_2 = self.emd(p2, gt_2)[0].sqrt().mean()
        else:
            dist1, dist2 = self.chamfer(pc, gt_c)
            loss_c = (dist1.sqrt().mean() + dist2.sqrt().mean()) / 2

            dist1, dist2 = self.chamfer(p1, gt_1)
            loss_1 = (dist1.sqrt().mean() + dist2.sqrt().mean()) / 2

            dist1, dist2 = self.chamfer(p2, gt_2)
            loss_2 = (dist1.sqrt().mean() + dist2.sqrt().mean()) / 2

        dist1, dist2 = self.chamfer(p3, gt_pcd)
        loss3 = (dist1.sqrt().mean() + dist2.sqrt().mean()) / 2

        dist1, _ = self.chamfer(inputs, p3)
        partial_matching = dist1.sqrt().mean()

        loss = loss_c + loss_1 + loss_2 + loss3 + partial_matching
        return loss
