...
def compute_prd(eval_dist, ref_dist, num_angles=1001, epsilon=1e-10):
    ...
    if not (0 < epsilon < 0.1):
        raise ValueError(f"epsilon must be in (0, 0.1] but is {epsilon!s}.")
    if not (3 <= num_angles <= 1e6):
        raise ValueError(f"num_angles must be in [3, 1e6] but is {num_angles}.")
...
def _cluster_into_bins(eval_data, ref_data, num_clusters):
    ...
    # sklearn stubs currently type this as a string literal (e.g. "auto").
    kmeans = sklearn.cluster.MiniBatchKMeans(n_clusters=num_clusters, n_init="auto")
    labels = np.asarray(kmeans.fit(cluster_data).labels_)
...
def compute_prd_from_embedding(
    eval_data, ref_data, num_clusters=20, num_angles=1001, num_runs=10, enforce_balance=True
):
    ...
    if enforce_balance and len(eval_data) != len(ref_data):
        raise ValueError(
            f"The number of points in eval_data {len(eval_data)} is not equal to the number of "
            f"points in ref_data {len(ref_data)}. To disable this exception, set enforce_balance "
...
def _prd_to_f_beta(precision, recall, beta: float = 1.0, epsilon=1e-10):
    ...
    if beta <= 0:
        raise ValueError(f"Given parameter beta {beta!s} must be positive.")
...
def plot(precision_recall_pairs, labels=None, out_path=None, legend_loc="lower left", dpi=300):
    ...
    if labels is not None and len(labels) != len(precision_recall_pairs):
        raise ValueError(
            f"Length of labels {len(labels)} must be identical to length of "
            f"precision_recall_pairs {len(precision_recall_pairs)}."
        )
